...
(defn run-cli-app [& args]
  (let [cli-args (cli/parse-opts
                   ...
                   [["-b" "--bootstrap-self-signed-certs" "In broker mode, bootstrap self-signed certificates."]
                    ...
                    ["-u" "--url URL"
                      "URL to bind the broker to. Supported protocols are: tcp, udp, mqtt, stomp, ws, ssl, stomp+ssl, mqtt+ssl, wss"
                    ...
                    ["-I" "--interval INTERVAL"
                      (str
                        "The interval in milliseconds with which, e.g., messages shall be generated by the message generator producer"
                        " or the delay that is artificially added to the consumption fn of the benchmark client."
                        " Note that this uses Thread/sleep and thus will be highly inaccurate for small values.")
                      :default 0
